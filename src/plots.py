...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.title("Relation between weight and freight value")
    plt.show()
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    for idx in holiday_dates:
        plt.axvline(x=idx, color="blue", linestyle="--", linewidth=0.5)
    sns.lineplot(x="date", y="order_count", data=df, color="darkgreen")
