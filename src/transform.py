...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    delivered = data.query(f"order_status=='delivered'")
    ...
    aggregations = delivered[["order_id", "freight_value", "product_weight_g"]]\
        .groupby(["order_id"])\
        .agg(sum)\
        .reset_index()
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    orders = orders.assign(
        order_purchase_timestamp = lambda x:pd.to_datetime
    ...
    order_purchase_ammount_per_date = filtered_dates[["order_purchase_timestamp", "order_id"]] \
    ...
    order_purchase_ammount_per_date["holiday"] = np.where(
        order_purchase_ammount_per_date["date"].isin(holidays["date"]), True, False)
